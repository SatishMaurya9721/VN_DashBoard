...
namespace DashBoard.Controllers
{
	...
	public class CategoryController : ControllerBase
	{
		...
		public async Task<IActionResult> InsertSubCategory(SubCategoryMaster model)
		{
			...
			var cmd = new SqlCommand(@"INSERT INTO SubCategoryMaster (CategoryId, FileName, PaymentType, Amount, FilePath,QRLink,DownloadLink) 
                                   VALUES (@CategoryId, @FileName, @PaymentType, @Amount, @FilePath,@QRLink,@DownloadLink)", conn);
			...
			cmd.Parameters.AddWithValue("@DownloadLink", model.DownloadLink);
			cmd.Parameters.AddWithValue("@QRLink", model.QRLink);
		...
		public IActionResult GetAllSubCategory(int CategoryId)
		{
			try
			{
				...
				var cmd = new SqlCommand("SELECT c.Name as CategoryName,scm.Id,scm.CategoryId,scm.FileName,scm.FilePath,scm.PaymentType,scm.Amount,scm.DownloadLink,scm.QRLink FROM SubCategoryMaster scm " +
				...
				while (reader.Read())
				{
					list.Add(new
					{
						...
						DownloadLink = reader["DownloadLink"]?.ToString(),
						QRLink = reader["QRLink"]?.ToString(),
